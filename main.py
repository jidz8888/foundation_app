...
def get_dashboard(db: Session = Depends(get_db), token: str = Depends(verify_token)):
    ...
    return {
        ...
        "totalDonations": {"title": "Total Donations", "value": f"Rs.{total_donations:,.0f}", "change": 0.0, "icon": "volunteer"},
        "recentActivities": [
            {
                "title": a.title,
...
def add_donation(request: DonationRequest, db: Session = Depends(get_db), token: str = Depends(verify_token)):
    ...
    log_activity(db, "New Donation Received", f"Rs.{new_donation.amount:,.0f} from {new_donation.donor_name}", "volunteer_activism")
